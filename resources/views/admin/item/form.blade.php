...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            @if (($item->uuid_item_inventory ?? '') == '')
                {!! Form::open(['url' => 'admin/items', 'method' => 'post', 'files'=>'true']) !!}
            @else
                {!! Form::model($item, ['url'=>'admin/inventory/'.$item->uuid_item_inventory, 'method' => 'PUT', 'files'=>'true']) !!}    
            @endif
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @else
                        <div class="form-group">
                            {!! Form::label('uuid_entry_note', 'ID Nota Masuk') !!}
                            {!! Form::text('uuid_entry_note', null, ['class'=>'form-control','placeholder'=>'Masukan ID Nota Masuk']) !!}
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        @foreach ($categories as $item)
                            @php $category[$item->id_category] = $item->name_category @endphp
                        @endforeach
                        {!! Form::label('id_category', 'Kategori') !!}
                        {!! Form::select('id_category', ($category ?? null), null, ['class'=>'form-control', 'placeholder'=>'Pilih Kategori']) !!}
                    </div>

                    <div class="form-group">
                        {!! Form::label('notes_item_inventory', 'Keterangan') !!}
                        {!! Form::textarea('notes_item_inventory', null, ['class'=>'form-control', 'placeholder'=>'Keterangan Item']) !!}
                    </div>

                    <div class="form-group">
                        {!! Form::label('picture_item_inventory', 'Foto Barang') !!} <br>
                        {!! Form::file('picture_item_inventory', ["accept"=>"image/*"]) !!}
                    </div>
